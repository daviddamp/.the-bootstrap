...
if [[ ! -n $(find ${FONTS_PATH} -iname ${FONT_NAME}) ]]; then
    ...
    mkdir -p "${FONTS_PATH}"
    curl -fLo "${FONTS_PATH}/${FONT_NAME}" 'https://github.com/ryanoasis/nerd-fonts/raw/master/patched-fonts/Hack/Regular/complete/Hack%20Regular%20Nerd%20Font%20Complete%20Mono.ttf'
fi
